...
function PlanGenerator({ user, imageAnalysis, userData, onComplete, onBack }: PlanGeneratorProps) {
  ...
  const handleGeneratePlan = async () => {
    ...
    try {
      ...
      console.log('DEBUG PLAN: Sending plan request:', JSON.stringify(planRequest, null, 2));

      const plan = await ApiClient.generatePlan(planRequest, user.access_token);
      
      console.log('DEBUG PLAN: Received plan:', JSON.stringify(plan, null, 2));
      
      onComplete(plan);
    } catch (err: any) {
      console.error('DEBUG PLAN: Error generating plan:', err);
